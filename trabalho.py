...
def main():
    ...
    print(classifica_times(ordena_pontos(times_compostos(jogos)), jogos))
...
def times_compostos(jogos: list) -> list:
    ...
    for j in range(0, len(jogos)):
        aux.append(Time(time_anfitriao(jogos[j]), conta_vitorias(time_anfitriao(jogos[j]), jogos), calcula_pontos(time_anfitriao(jogos[j]), jogos), saldo_gols(time_anfitriao(jogos[j]), jogos)))
    for time in aux:
